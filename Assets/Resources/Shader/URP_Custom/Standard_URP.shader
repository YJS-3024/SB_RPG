Shader "URP_Custom/Standard_URP"
{
    Properties
    {
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            half4 Frag(Varyings input) : SV_Target
            {
                ...
                half4 metalGloss = SAMPLE_TEXTURE2D(_MetallicGlossMap, sampler_MetallicGlossMap, input.uvMetallic);
                half metallic = saturate(_Metallic * metalGloss.r);
                half smoothnessFromTex = lerp(metalGloss.a, baseSample.a, saturate(_SmoothnessTextureChannel));
                ...
                half3 diffuseColor = baseSample.rgb * (1.0h - metallic * 0.6h);
                half3 specColor = lerp(0.04h.xxx, baseSample.rgb, metallic);

                float4 shadowCoord = TransformWorldToShadowCoord(input.positionWS);
                Light mainLight = GetMainLight(shadowCoord);
                half ndotl = saturate(dot(normalWS, mainLight.direction));

                half3 color = diffuseColor * SampleSH(normalWS) * occlusion;
                color += diffuseColor * mainLight.color * ndotl * mainLight.shadowAttenuation * occlusion;
                ...
                for (uint lightIndex = 0u; lightIndex < pixelLightCount; ++lightIndex)
                {
                    ...
                    color += diffuseColor * light.color * lightNdotL * light.distanceAttenuation * light.shadowAttenuation * occlusion;
                }
                #endif
